...
export class ComicController {
	...
	findProblematicComics = async (
		...
		try {
			...
			console.log(`Controller: findProblematicComics called by user ${internalUserId}`);
			
			const result = await this.comicService.findProblematicComics(internalUserId);
			res.status(200).json(result);
		} catch (error: any) {
			console.error("Error in findProblematicComics controller:", error.message);
			res
				.status(500)
				.json({ error: error.message || "Failed to find problematic comics." });
		}
	};

	/**
	 * Handles requests to delete a comic by ID for the authenticated user.
	 */
	deleteComic = async (
		req: AuthenticatedRequest,
		res: Response
		...
		try {
			...
			// Delete the comic (this will cascade to related tables)
			const result = await this.comicService.deleteComic(comicId, internalUserId);
			...
			console.error(
				`Error in deleteComic controller for comic ${req.params.comicId}:`,
				error.message
			);
			res
				.status(500)
				.json({ error: error.message || "Failed to delete comic." });
		}
	};
}
