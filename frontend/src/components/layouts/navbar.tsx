...
export default function Navbar() {
  ...
  const renderAuthLinks = (isMobile = false) => {
    ...
    if (isLoading) {
      return (
        <div className={`flex items-center ${isMobile ? 'px-4 py-2 h-[36px]' : 'ml-6 h-[36px]'}`}>
    ...
    if (user) {
      return (
        <>
          <Link href="/profile" onClick={closeMobileMenu} className={isMobile ? "flex items-center pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800" : "inline-flex items-center px-2 py-1 text-sm font-medium text-gray-500 hover:text-gray-700 rounded-md ml-4"} >
            <User className="h-4 w-4 mr-1" /> Profile
          </Link>
          <Button variant="ghost" size="sm" onClick={async () => { closeMobileMenu(); await handleSignOut(); }} className={isMobile ? "w-full text-left flex items-center pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800" : "ml-2"} >
      ...
      return (
        <>
          <Link href="/login" onClick={closeMobileMenu} className={isMobile ? "flex items-center pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800" : "inline-flex items-center px-3 py-1 border border-transparent text-sm font-medium rounded-md text-gray-700 bg-gray-100 hover:bg-gray-200 ml-4"} >
            <LogIn className="h-4 w-4 mr-1" /> Login
          </Link>
          <Link href="/signup" onClick={closeMobileMenu} className={isMobile ? "flex items-center pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800" : "inline-flex items-center px-3 py-1 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 ml-2"} >
  ...
  const renderCreateLink = (isMobile = false) => {
     if (isLoading) {
         return ( <span className={isMobile ? "flex items-center pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-400 cursor-not-allowed" : "inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium text-gray-400 cursor-not-allowed"} > <PlusCircle className="h-4 w-4 mr-1" /> Create </span> );
     }
     if (user) {
         return ( <Link href="/comics/create" onClick={closeMobileMenu} className={isMobile ? "flex items-center pl-3 pr-4 py-2 border-l-4 border-blue-500 bg-blue-50 text-base font-medium text-blue-700" : "inline-flex items-center px-1 pt-1 border-b-2 border-blue-500 text-sm font-medium text-blue-600"} > <PlusCircle className="h-4 w-4 mr-1" /> Create </Link> );
     } else {
         return ( <span className={isMobile ? "flex items-center pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-400 cursor-not-allowed" : "inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium text-gray-400 cursor-not-allowed"} title="Login to create comics" > <PlusCircle className="h-4 w-4 mr-1" /> Create </span> );
     }
  };
        ...
        <div className="flex justify-between h-16">
          ...
          <div className="flex">
            <div className="flex-shrink-0 flex items-center">
              <Link href="/" className="font-bold text-xl text-blue-600"> Comic Creator </Link>
            </div>
          ...
          <div className="-mr-2 flex items-center sm:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500" aria-controls="mobile-menu" aria-expanded={isOpen} >
